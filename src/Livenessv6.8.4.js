...
class Liveness {
    ...
    this.timer = setInterval(async () => {
      ...
      if (resizedDetections.detection) {
        ...
        if (!this.ignoreFaceRules) {
          ...
          if (this.isFaceCloser(leftEye.meanPosition, rightEye.meanPosition, positions)) {
          ...
          if (this.isNotBetweenEyesBoxes(leftEye.meanPosition, rightEye.meanPosition, positions)) {
            this.blockMaskMessage = this.boxMessages.moveFaceCloser
            this.blockMask(canvasPosition, frameBox.left, frameBox.top, frameBox.height, frameBox.width)
            state.counter = 0
            state.inProgress = false
  ...
  blockMask (canvasPosition, left, top, height, width) {
    ...
    elMessage.ariaLabel = this.blockMaskMessage === 'Alinhe seu rosto à tela'
      ? 'Seu rosto está inclinado, deixe-o alinhado à tela'
